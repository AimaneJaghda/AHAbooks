    ...
    try{
        ...
        if($errHandling){
            ...
            if(file_exists($_FILES["photoProfil"]["tmp_name"])){
                $typeFichierSelection = $_FILES['photoProfil']['type'];
                $exts=array("image/png","image/jpeg");
                if(in_array($typeFichierSelection,$exts)){
        
                    $nomfichier = $_FILES['photoProfil']['name'];
                    $tempFichier = $_FILES['photoProfil']['tmp_name'];
                ...
                $location = "users\defaultProfilePicture.jpg";
            }
            
                
            $requite = $connexion -> prepare("INSERT INTO AHAuser(email, passwordUser, lastName, firstName, birthDate, creationDate, city, country, typeUser, profilePicture) VALUES(?, ?, ?, ?, ?, CURRENT_DATE(), ?, ?, ?, ?)");
            $requite -> execute(array($email, $password, $lname, $fname, $datenaissance, $city, $contry, "user", $location));
