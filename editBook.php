<?php
    ...
    $requite = $connexion -> prepare("SELECT * FROM AHAbook WHERE idBook=?");
    $requite -> execute([$_POST["idBook"]]);
    $line = $requite -> fetch();

    $categ = explode(" ", $line["categoryBook"]);
...
<body>
    ...
    <div class="signup rounded-3 shadow-lg py-4 mb-5">
        <form action="editBookAction.php" method="post" enctype="multipart/form-data">
            <input type="hidden" name="idBook" value="<?php echo $_POST["idBook"]; ?>">
            <div class="form-group my-2">
                <label for="" class="form-label">Titre de Livre</label>
                <input type="text" name="titreBook" id="" class="form-control" value="<?php echo $line['titreBook'] ?>">
            </div>
            <div class="d-flex">
                <div class="form-group w-50 pe-3">
                    <label for="" class="form-label">Prix</label>
                    <input type="text" name="prixBook" id="" class="form-control" value="<?php echo $line['price'] ?>">
                </div>
                <div class="form-group w-50">
                    <label for="" class="form-label">Numbre des pages</label>
                    <input type="text" name="NpageBook" id="" class="form-control" value="<?php echo $line['numberPages'] ?>">
                </div>
            </div>
            <div class="form-group my-2">
                <label for="" class="form-label">Editeur de Livre</label>
                <input type="text" name="editorBook" id="" class="form-control" value="<?php echo $line['editorBook'] ?>">
            </div>
            <div class="d-flex">
                <div class="form-group w-50 pe-3">
                    <label for="" class="form-label">ISBN</label>
                    <input type="text" name="isbnBook" id="" class="form-control" value="<?php echo $line['ISBN'] ?>">
                </div>
                <div class="form-group w-50">
                    <label for="" class="form-label">Date de publication</label>
                    <input type="date" name="DPBook" id="" class="form-control" value="<?php echo $line['publicationDate'] ?>">
                </div>
            </div>
            <div class="form-group my-4">
            <div class="d-flex justify-content-between">
                    <div>
                        <input type="checkbox" name="category[]" value="Fiction" id="" class="form-check-input" <?php if(in_array("Fiction",$categ)){echo "checked";} ?>>
                        <span>Fiction</span>
                    </div>
                    <div>
                        <input type="checkbox" name="category[]" value="Fantasy" id="" class="form-check-input"<?php if(in_array("Fantasy",$categ)){echo "checked";} ?>>
                        <span>Fantasy</span>
                    </div>
                    <div>
                        <input type="checkbox" name="category[]" value="Mystery" id="" class="form-check-input" <?php if(in_array("Mystery",$categ)){echo "checked";} ?>>
                        <span>Mystery</span>
                    </div>
                    <div>
                        <input type="checkbox" name="category[]" value="Romance" id="" class="form-check-input" <?php if(in_array("Romance",$categ)){echo "checked";} ?>>
                        <span>Romance</span>
                    </div>
                    <div>
                        <input type="checkbox" name="category[]" value="History" id="" class="form-check-input" <?php if(in_array("History",$categ)){echo "checked";} ?>>
                        <span>History</span>
                    </div>
                </div>
            </div>
            <div class="form-group my-2">
                ...
                <select class="form-control" name="langue" id="langue">
                    ...
                    <option value="Français" <?php if($line['langueBook'] == "Français"){ echo "selected"; } ?>>Français</option>
                    <option value="Arabic" <?php if($line['langueBook'] == "Arabic"){ echo "selected"; } ?>>Arabic</option>
                    <option value="English" <?php if($line['langueBook'] == "English"){ echo "selected"; } ?>>English</option>
                    <option value="Espagnole" <?php if($line['langueBook'] == "Espagnole"){ echo "selected"; } ?>>Espagnole</option>
                </select>
            </div>
            <div class="form-group my-2">
                <label for="" class="form-label">Photo</label>
                <input type="file" name="bookPicture" id="photo" class="form-control" value="<?php echo $line["bookPicture"]; ?>" required>
            </div>
            <div class="form-group my-2">
                <label for="description" class="form-label">Description</label>
                <textarea class="form-control" name="descriptionBook" id="" rows="3"><?php echo $line['bookDescription'] ?></textarea>
            </div>
            <div class="form-group mt-4 text-center">
                <input type="submit" value="Modifier Livre" name="modifier" class="form-control btn btn-outline-warning my-1" id="submitBtn">
                <hr>
            </div>
        </form>
    </div>
